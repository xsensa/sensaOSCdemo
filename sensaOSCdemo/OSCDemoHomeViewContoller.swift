...
class OSCDemoHomeViewController: UIViewController, OSCServerDelegate , UIPopoverPresentationControllerDelegate  {
    ...
    @IBAction func onSettings(_ sender: Any) {
        ...
        let storyboard: UIStoryboard = UIStoryboard(name: "mainOSCDemo", bundle: nil)
        let destination = storyboard.instantiateViewController(withIdentifier:  "OSCSettingsTableViewController") as! OSCSettingsTableViewController
        ...
        for i in 0..<users.count {
            if userviews[i].alpha > 0.3  { // broadcast to active users only
                ...
                for n in 0..<networks.count {
                    ...
                    Message.text = ("sending /hub\(networks[n])/  \(hubNetMsgs[n])" )
